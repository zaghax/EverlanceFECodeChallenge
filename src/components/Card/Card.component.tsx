...
const Card = ({ cardInfo }: propsTypes) => {
  ...
  return (
    <div className='aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-md lg:aspect-none p-4 border-solid border'>
      <div className='flex items-center mb-4'>
        <img
          src={cardInfo.avatar_url}
          alt={cardInfo.login}
          className='inline-block h-16 w-16 rounded-full ring-2 ring-white mr-2'
        />
        <p className='font-semibold text-gray-600'>{cardInfo.login}</p>
      </div>
      <div className='flex justify-between'>
        ...
        <button type='button' onClick={setSelectedUser} className='font-semibold text-cyan-400'>
          View profile
        </button>
      </div>
    </div>
